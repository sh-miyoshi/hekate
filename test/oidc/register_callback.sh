...
# Token get
token_info=`curl --insecure -s -X POST http://localhost:18443/api/v1/project/master/openid-connect/token \
...
curl --insecure -s -X PUT \
  ...
  http://localhost:18443/api/v1/project/master/client/portal
