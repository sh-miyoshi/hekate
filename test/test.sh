...
token_info=`curl -s -X POST -d '@token_request.json' $URL/project/$PROJECT_ID/token`
...
EOF
new_token_info=`curl -s -X POST -d '@new_token_request.json' $URL/project/$PROJECT_ID/token`
