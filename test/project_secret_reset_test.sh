...
# Get Public Key Info
prev=`curl -k -s -X GET -H "Authorization: Bearer $master_access_token" "$SERVER_ADDR/authapi/v1/project/master/openid-connect/certs" | jq -r .keys[0].n`

# # Project Secret Reset
curl -k -s -X POST -H "Authorization: Bearer $master_access_token" "$URL/project/master/keys/reset"

# Get Public Key Info
current=`curl -k -s -X GET -H "Authorization: Bearer $master_access_token" "$SERVER_ADDR/authapi/v1/project/master/openid-connect/certs" | jq -r .keys[0].n`
