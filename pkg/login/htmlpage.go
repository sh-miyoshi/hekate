...
func WriteUserLoginPage(projectName, sessionID, errMsg, state string, w http.ResponseWriter) {
	...
	url := "/adminapi/v1/project/" + projectName + "/authn/login?login_session_id=" + sessionID
	if state != "" {
		url += "&state=" + state
	}
...
func WriteConsentPage(projectName, sessionID, state string, w http.ResponseWriter) {
	...
	url := "/adminapi/v1/project/" + projectName + "/authn/consent?login_session_id=" + sessionID
	if state != "" {
		url += "&state=" + state
	}
